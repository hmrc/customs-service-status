  ...
  "POST /services/messages" should {
    "return None if the information insert to the database was acknowledged" in {
      val result =
        await(
          callRoute(
            fakeRequest(routes.AdminCustomsServiceStatusController.updateWithOutageData()).withBody(Json.toJson(validOutageData))
          )
        )
      ...
      contentAsJson(findResult).as[List[OutageData]] shouldBe List(validOutageData)
    }
